...
剛剛 python 的 DataFrame 可以用下面的方式分割後存入資料表中：
```python
...
***
## 先計算還是即時計算
在 MariaDB 中建立龐大的資料庫之後，就可以利用他的好朋友 PHP 來對資料庫查詢並計算結果了。為了產生圖表，可以稍微想像一下要產生的資料樣貌，大概會是連續不中斷的日期對上總和，如下：
```php
array(
    "日期" => 數量,
    "日期" => 數量,
...
```
如果要產生上面的陣列，可以在 php 中連接資料庫後直接用 MySQL SUM GROUP BY 來計算總和：
```php
...
```
但是無法保證上面產出的日期是連續日期。例如禮拜天如果沒有消耗資料，就完全不會有該天的資料，並不會顯示 `禮拜天 => 0` ，而是直接從禮拜六就跳到禮拜一，為了避免這樣的狀況，要重新建立一個具有連續日期的陣列，再把舊陣列資料儲存進去。
```php
...
```
如此就可以簡單快速的完成圖表需要的資料，接下來只要把資料塞進 chartjs 中就可以視覺化了。
